...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      <th scope="row">{{$i++}}</th>
      @php 
      $pic = $post->picture ? $post->picture->name :false;
      
      @endphp
      <td> @if($pic)
        <img src="{{asset('images/'.$pic)}}" style="width:150px">
      @else
      ...
      <form action="{{route('admin.post.delete', $post->id)}}" method="post">
        ...
        @csrf
        @php 
        $pic_id = $post->picture ? $post->picture->id : 0;
        @endphp
        <input type="hidden" name="picture_id" value="{{$pic_id}}">
